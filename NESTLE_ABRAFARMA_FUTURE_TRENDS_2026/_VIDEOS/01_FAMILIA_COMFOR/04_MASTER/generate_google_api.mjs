...
async function main() {
  const sceneId = "S01";
  const jsonFile = fs.readdirSync(MASTER_DIR).find(f => f.startsWith(sceneId + "_") && f.endsWith(".json") && !f.includes("BATCH"));
  ...
  try {
    ...
    const metaPath = path.join(RAW_OUTPUT_DIR, `${sceneId}_${ts}_google_api_operation.json`);
...
Aguardando conclusao (pode demorar alguns minutos)...`);
    ...
    while (!finished && attempts < 100) {
      ...
      process.stdout.write(`  [${new Date().toLocaleTimeString()}] Status: ${data.metadata?.state || data.done}   \r`);
      ...
      if (data.done) {
        ...
        if (data.response && data.response.videoUri) {
          ...
          const outFile = path.join(RAW_OUTPUT_DIR, `${sceneId}_FT26-NAN-COMFOR_${ts}_veo3_google.mp4`);
          fs.writeFileSync(outFile, vidBuffer);
          console.log(`Video salvo com sucesso em: ${outFile} (${(vidBuffer.length / 1024 / 1024).toFixed(2)} MB)`);
        } else if (data.error) {
          console.error(`\n❌ Erro retornado pela API:`, data.error);
        } else {
          console.log(`Não foi possível localizar o URI do vídeo na resposta:`, JSON.stringify(data, null, 2));
        }
        finished = true;
      }
    }
