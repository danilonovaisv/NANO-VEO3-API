...
async function main() {
  ...
  try {
    ...
    console.log(`  Operation State: ${operation.metadata?.state || 'PROCESSANDO'}`);
    
    // Save operation ticket
    fs.mkdirSync(RAW_OUTPUT_DIR, { recursive: true });
    const ts = new Date().toISOString().replace(/[:.]/g, "-");
    const metaPath = path.join(RAW_OUTPUT_DIR, `${sceneId}_${ts}_vertex_operation.json`);
...
Aguardando conclusao (pode demorar alguns minutos)...`);
    ...
    if (operation.videoUri || operation.uri || operation.generatedVideo) {
      console.log(`🎉 Geração concluída! URL:`, operation);
        ...
        console.log("⚠️ Operação em andamento no Vertex AI, verifique o console do GCP para o progresso.");
    }
