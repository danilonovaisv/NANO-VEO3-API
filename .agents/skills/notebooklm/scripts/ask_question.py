...
# Ensure .venv site-packages is in sys.path if patchright is not in global interpreter
try:
    import patchright
except ImportError:
    _venv_site = list(Path(__file__).parent.parent.glob(".venv/lib/python*/site-packages"))
    if _venv_site:
...
def ask_notebooklm(question: str, notebook_url: str, headless: bool = True) -> str:
    ...
    try:
        ...
        # Wait for NotebookLM
        page.wait_for_url(re.compile(r"^https://notebook(lm)?\.google\.com/"), timeout=15000)
